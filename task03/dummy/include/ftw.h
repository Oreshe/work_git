...
#define FTW_DEPTH FTW_DEPTH
	...
	,
	FTW_ACTIONRETVAL =
		16 /* Assume callback to return FTW_* values instead of
...
#define FTW_STOP FTW_STOP
	FTW_SKIP_SUBTREE =
		2, /* Only meaningful for FTW_D: Don't walk through the
			   subtree, instead just continue with its next
			   sibling. */
#define FTW_SKIP_SUBTREE FTW_SKIP_SUBTREE
	FTW_SKIP_SIBLINGS =
		3, /* Continue with FTW_DP callback for current directory
...
extern int __REDIRECT(ftw,
		      (const char *__dir, __ftw_func_t __func,
		       int __descriptors),
		      ftw64) __nonnull((1, 2));
#else
extern int __REDIRECT(ftw,
...
#else
#ifdef __REDIRECT
extern int __REDIRECT(ftw64,
		      (const char *__dir, __ftw64_func_t __func,
		       int __descriptors),
		      __ftw64_time64) __nonnull((1, 2));
#else
