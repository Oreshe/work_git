...
#define SUDO_API_VERSION_SET_MAJOR(vp, n)                    \
	do {                                                 \
		*(vp) = (*(vp) & 0x0000ffffU) | ((n) << 16); \
	} while (0)
#define SUDO_API_VERSION_SET_MINOR(vp, n)            \
	do {                                         \
		*(vp) = (*(vp) & 0xffff0000U) | (n); \
	} while (0)
...
#define SUDO_CONV_CALLBACK_VERSION                           \
	SUDO_API_MKVERSION(SUDO_CONV_CALLBACK_VERSION_MAJOR, \
...
typedef int (*sudo_hook_fn_t)();
#endif
typedef int (*sudo_hook_fn_setenv_t)(const char *name, const char *value,
				     int overwrite, void *closure);
typedef int (*sudo_hook_fn_putenv_t)(char *string, void *closure);
typedef int (*sudo_hook_fn_getenv_t)(const char *name, char **value,
				     void *closure);
...
#define SUDO_POLICY_PLUGIN 1
	...
	int (*list)(int argc, char *const argv[], int verbose, const char *user,
		    const char **errstr);
	...
	void (*deregister_hooks)(
		int version, int (*deregister_hook)(struct sudo_hook *hook));
