...
struct sudo_plugin_event {
  int (*set)(struct sudo_plugin_event* pev,
             int fd,
             int events,
  ...
  int (*pending)(struct sudo_plugin_event* pev,
                 int events,
                 struct timespec* ts);
...
#define SUDO_IO_PLUGIN 2
  ...
  void (*deregister_hooks)(int version,
                           int (*deregister_hook)(struct sudo_hook* hook));
  int (*change_winsize)(unsigned int line,
                        unsigned int cols,
                        const char** errstr);
...
#define SUDO_AUDIT_PLUGIN 3
  ...
  int (*accept)(const char* plugin_name,
                ...
                char* const run_envp[],
                const char** errstr);
  int (*reject)(const char* plugin_name,
                unsigned int plugin_type,
                const char* audit_msg,
                char* const command_info[],
                const char** errstr);
  int (*error)(const char* plugin_name,
               unsigned int plugin_type,
               const char* audit_msg,
               char* const command_info[],
               const char** errstr);
...
#define SUDO_APPROVAL_PLUGIN 4
  ...
  int (*open)(unsigned int version,
              ...
              const char** errstr);
  void (*close)(void);
  int (*check)(char* const command_info[],
               char* const run_argv[],
               char* const run_envp[],
               const char** errstr);
  int (*show_version)(int verbose);
