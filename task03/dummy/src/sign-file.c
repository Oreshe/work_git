...
static __attribute__((noreturn)) void format(void) {
  fprintf(stderr,
          "Usage: scripts/sign-file [-dp] <hash algo> <key> <x509> <module> "
          "[<dest>]\n");
  fprintf(stderr,
...
int main(int argc, char** argv) {
  ...
  if (!raw_sig) {
    ...
    unsigned int flags = CMS_NOCERTS | CMS_NOATTR | CMS_PARTIAL | CMS_BINARY |
    ...
    if (save_sig) {
      ...
      ERR(asprintf(&sig_file_name, "%s.p7s", module_name) < 0, "asprintf");
      b = BIO_new_file(sig_file_name, "wb");
      ERR(!b, "%s", sig_file_name);
      ERR(i2d_CMS_bio_stream(b, cms, NULL, 0) != 1, "%s", sig_file_name);
      BIO_free(b);
    }
