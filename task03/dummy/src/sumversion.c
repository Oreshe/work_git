...
#define ROUND2(a, b, c, d, k, s) \
	(a = lshift(a + G(b, c, d) + k + (uint32_t)0x5A827999, s))
#define ROUND3(a, b, c, d, k, s) \
	(a = lshift(a + H(b, c, d) + k + (uint32_t)0x6ED9EBA1, s))
...
static void md4_update(struct md4_ctx *mctx, const unsigned char *data,
		       unsigned int len)
{
...
static void md4_final_ascii(struct md4_ctx *mctx, char *out, unsigned int len)
{
	...
	*p++ = 0x80;
	...
	le32_to_cpu_array(mctx->block,
			  (sizeof(mctx->block) - sizeof(uint64_t)) /
				  sizeof(uint32_t));
	md4_transform(mctx->hash, mctx->block);
	cpu_to_le32_array(mctx->hash, ARRAY_SIZE(mctx->hash));

	snprintf(out, len, "%08X%08X%08X%08X", mctx->hash[0], mctx->hash[1],
		 mctx->hash[2], mctx->hash[3]);
}
...
static int parse_string(const char *file, unsigned long len, struct md4_ctx *md)
{
	unsigned long i;
...
static int parse_source_files(const char *objfile, struct md4_ctx *md)
{
	...
	while ((line = get_line(&pos))) {
		...
		if ((strstr(line, dir) + strlen(dir) - 1) ==
		    strrchr(line, '/')) {
