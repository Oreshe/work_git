...
static inline void add_uuid(char* str, uuid_le uuid) {
  ...
  sprintf(
      ...
      uuid.b[3], uuid.b[2], uuid.b[1], uuid.b[0], uuid.b[5], uuid.b[4],
      uuid.b[7], uuid.b[6], uuid.b[8], uuid.b[9], uuid.b[10], uuid.b[11],
...
static inline void add_guid(char* str, guid_t guid) {
  ...
  sprintf(
      str + len,
      "%02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X",
      guid.b[3], guid.b[2], guid.b[1], guid.b[0], guid.b[5], guid.b[4],
      guid.b[7], guid.b[6], guid.b[8], guid.b[9], guid.b[10], guid.b[11],
...
static void do_usb_entry(void* symval,
  ...
  if (range_lo == range_hi)
    ...
    if (range_lo > 0x9 || range_hi < 0xA)
      sprintf(alias + strlen(alias), "[%X-%X]", range_lo, range_hi);
    else {
      sprintf(alias + strlen(alias), range_lo < 0x9 ? "[%X-9" : "[%X",
              range_lo);
      sprintf(alias + strlen(alias), range_hi > 0xA ? "A-%X]" : "%X]",
              range_hi);
    }
  }
  ...
  ADD(alias, "dc", match_flags & USB_DEVICE_ID_MATCH_DEV_CLASS, bDeviceClass);
...
static void do_usb_entry_multi(struct module* mod, void* symval) {
  ...
  for (ndigits = sizeof(bcdDevice_lo) * 2 - 1; devlo <= devhi; ndigits--) {
    ...
    if (clo > 0x0)
      do_usb_entry(symval, incbcd(&devlo, 1, max, sizeof(bcdDevice_lo) * 2),
                   ndigits, clo, max, max, mod);

    if (chi < max)
      do_usb_entry(symval, incbcd(&devhi, -1, max, sizeof(bcdDevice_lo) * 2),
...
static void do_of_entry(struct module* mod, void* symval) {
  ...
  if ((*compatible)[0])
    sprintf(&alias[len], "%sC%s", (*type)[0] ? "*" : "", *compatible);
...
static void do_ieee1394_entry(struct module* mod, void* symval) {
  ...
  ADD(alias, "sp", match_flags & IEEE1394_MATCH_SPECIFIER_ID, specifier_id);
...
static void do_pci_entry(struct module* mod, void* symval) {
  ...
  unsigned char baseclass, subclass, interface, baseclass_mask, subclass_mask,
      interface_mask;
...
static void do_ccw_entry(struct module* mod, void* symval) {
  ...
  ADD(alias, "dt", match_flags & CCW_DEVICE_ID_MATCH_DEVICE_TYPE, dev_type);
  ADD(alias, "dm", match_flags & CCW_DEVICE_ID_MATCH_DEVICE_MODEL, dev_model);
...
static void do_acpi_entry(struct module* mod, void* symval) {
  ...
  if ((*id)[0])
    ...
    for (i = 1; i <= 3; i++) {
      ...
      if (msk)
        sprintf(&alias[cnt], "%02x", (cls >> byte_shift) & 0xFF);
      else
        sprintf(&alias[cnt], "??");
      cnt += 2;
    }
...
static void do_pcmcia_entry(struct module* mod, void* symval) {
  ...
  ADD(alias, "pfn", match_flags & PCMCIA_DEV_ID_MATCH_DEVICE_NO, device_no);
  ...
  for (i = 0; i < ARRAY_SIZE(dmi_fields); i++) {
    for (j = 0; j < 4; j++) {
      if ((*matches)[j].slot && (*matches)[j].slot == dmi_fields[i].field) {
        sprintf(alias + strlen(alias), ":%s*", dmi_fields[i].prefix);
        dmi_ascii_filter(alias + strlen(alias), (*matches)[j].substr);
        strcat(alias, "*");
      }
    }
  }
...
static void do_isapnp_entry(struct module* mod, void* symval) {
  ...
  module_alias_printf(
      mod, false, "pnp:d%c%c%c%x%x%x%x*", 'A' + ((vendor >> 2) & 0x3f) - 1,
      'A' + (((vendor & 3) << 3) | ((vendor >> 13) & 7)) - 1,
      'A' + ((vendor >> 8) & 0x1f) - 1, (function >> 4) & 0x0f, function & 0x0f,
...
static void do_amba_entry(struct module* mod, void* symval) {
  ...
  if ((id & mask) != id)
    fatal(
        "%s: Masked-off bit(s) of AMBA device ID are non-zero: id=0x%08X, "
        "mask=0x%08X.  Please fix this driver.\n",
        mod->name, id, mask);
...
static void do_tbsvc_entry(struct module* mod, void* symval) {
  ...
  ADD(alias, "v", match_flags & TBSVC_MATCH_PROTOCOL_VERSION, protocol_version);
...
static void do_tee_entry(struct module* mod, void* symval) {
  ...
  module_alias_printf(mod, true,
                      "tee:%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%"
                      "02x%02x%02x%02x%02x",
...
static void do_ssam_entry(struct module* mod, void* symval) {
  ...
  module_alias_printf(mod, false, "ssam:d%02Xc%02X%s", domain, category, alias);
}
...
static void do_table(const char* name,
                     void* symval,
                     unsigned long size,
                     unsigned long id_size,
                     const char* device_id,
...
void handle_moddevtable(struct module* mod,
  ...
  for (int i = 0; i < ARRAY_SIZE(devtable); i++) {
    ...
    if (sym_is(type, typelen, p->device_id)) {
      do_table(name, symval, sym->st_size, p->id_size, p->device_id,
               p->do_entry, mod);
      break;
    }
  }
