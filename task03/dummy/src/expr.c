...
static struct expr* expr_lookup(enum expr_type type, void* l, void* r) {
  ...
  hash_for_each_possible(expr_hashtable, e, node, hash) {
    if (e->type == type && e->left._initdata == l && e->right._initdata == r)
      return e;
  }
...
struct expr* expr_transform(struct expr* e) {
  ...
  switch (e->type) {
    ...
    case E_SYMBOL:
      break;
    default:
      e = expr_alloc_two(e->type, expr_transform(e->left.expr),
    ...
    case E_NOT:
      switch (e->left.expr->type) {
        ...
        case E_UNEQUAL:
          // !(A=B) -> A!=B
          e = expr_alloc_comp(
              e->left.expr->type == E_EQUAL ? E_UNEQUAL : E_EQUAL,
              e->left.expr->left.sym, e->left.expr->right.sym);
          break;
        case E_LEQ:
        case E_GEQ:
          // !(A<=B) -> A>B
...
void expr_invalidate_all(void) {
  ...
  hash_for_each(expr_hashtable, e, node) e->val_is_valid = false;
}
...
void expr_gstr_print_revdep(struct expr* e,
                            struct gstr* gs,
                            tristate pr_type,
                            const char* title) {
