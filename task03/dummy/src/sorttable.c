...
static void* sort_orctable(void* arg) {
  ...
  if (!idxs) {
    snprintf(g_err, ERRSTR_MAXSZ, "malloc idxs: %s", strerror(errno));
    pthread_exit(g_err);
  }
  ...
  if (!tmp_orc_table) {
    snprintf(g_err, ERRSTR_MAXSZ, "malloc tmp_orc_table: %s", strerror(errno));
    pthread_exit(g_err);
  }
...
static int fill_relocs(void* ptr,
                       uint64_t size,
                       Elf_Ehdr* ehdr,
                       uint64_t start_loc) {
  Elf_Shdr* shdr_start;
    ...
    for (; (void*)rel < end; rel = (void*)rel + shdr_entsize(shdr)) {
      ...
      if (offset >= start_loc && offset < start_loc + size) {
        if (ptr + long_size > array_end) {
          snprintf(m_err, ERRSTR_MAXSZ, "Too many relocations");
          return -1;
        }

        /* Make sure this has the correct type */
        if (rela_info(rel) != rela_type) {
          snprintf(m_err, ERRSTR_MAXSZ, "rela has type %lx but expected %lx\n",
...
static void replace_relocs(void* ptr,
                           uint64_t size,
                           Elf_Ehdr* ehdr,
                           uint64_t start_loc) {
  Elf_Shdr* shdr_start;
...
static void get_mcount_loc(struct elf_mcount_loc* emloc,
                           Elf_Shdr* symtab_sec,
                           const char* strtab) {
  Elf_Sym *sym, *end_sym;
...
static int do_sort(Elf_Ehdr* ehdr,
  ...
  for (i = 0; i < shnum; i++) {
    ...
    if (shdr_type(shdr) == SHT_SYMTAB_SHNDX)
      symtab_shndx = (Elf32_Word*)((const char*)ehdr + shdr_offset(shdr));
    ...
    if (!strcmp(secstrings + idx, ".orc_unwind_ip")) {
      ...
      g_orc_ip_table = (int*)((void*)ehdr + shdr_offset(shdr));
    }
    if (!strcmp(secstrings + idx, ".orc_unwind")) {
      orc_size = shdr_size(shdr);
      g_orc_table = (struct orc_entry*)((void*)ehdr + shdr_offset(shdr));
    }
#endif
  } /* for loop */
  ...
  if (orc_ip_size % sizeof(int) != 0 ||
    ...
    fprintf(stderr, "inconsistent ORC unwind table entries in file: %s\n",
            fname);
    goto out;
  }

  /* create thread to sort ORC unwind tables concurrently */
  if (pthread_create(&orc_sort_thread, NULL, sort_orctable, &orc_ip_size)) {
    fprintf(stderr, "pthread_create orc_sort_thread failed '%s': %s\n",
  ...
  if (pthread_create(&mcount_sort_thread, NULL, &sort_mcount_loc, &mstruct)) {
    fprintf(stderr, "pthread_create mcount_sort_thread failed '%s': %s\n",
            strerror(errno), fname);
    goto out;
  }
#endif
  ...
  if (custom_sort) {
    ...
    qsort(extab_image, num_entries, extable_ent_size, compare_extable);
  }
  ...
  for (sym = sym_start; (void*)sym + symentsize < sym_end;
    ...
    if (sym_type(sym) != STT_OBJECT)
      continue;
    if (!strcmp(strtab + sym_name(sym), "main_extable_sort_needed")) {
      sort_needed_sym = sym;
      break;
    }
  }
  ...
  if (!sort_needed_sym) {
    fprintf(stderr, "no main_extable_sort_needed symbol in file: %s\n", fname);
    goto out;
  }

  sort_need_index = get_secindex(
      sym_shndx(sym), ((void*)sort_needed_sym - (void*)symtab) / symentsize,
      symtab_shndx);
