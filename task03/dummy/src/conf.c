  ...
  while (cnt > 0) {
    ...
    menu_for_each_sub_entry(menu, choice) {
      ...
      if (x < 0) {
         ...
         */
        list_move_tail(&sym->choice_link, &choice->choice_members);
  ...
  while (1) {
    ...
    for (child = menu->list; child; child = child->next) {
      ...
      if (child->sym == def_sym) {
        ...
        printf("%*c", indent, ' ');
      printf(" %d. %s (%s)", cnt, menu_get_prompt(child), child->sym->name);
...
static void conf_usage(const char* progname) {
  ...
  printf(
      "  --oldaskconfig          Start a new configuration using a "
      "line-oriented program\n");
  printf(
      "  --oldconfig             Update a configuration using a provided "
      ".config as base\n");
  printf(
      "  --syncconfig            Similar to oldconfig but generates "
      "configuration in\n"
      "                          include/{generated/,config/}\n");
  printf(
      "  --olddefconfig          Same as oldconfig but sets new symbols to "
      "their default value\n");
  printf(
      "  --defconfig <file>      New config with default defined in <file>\n");
  printf(
      "  --savedefconfig <file>  Save the minimal current configuration to "
      "<file>\n");
  printf(
      "  --allnoconfig           New config where all options are answered "
      "with no\n");
  printf(
      "  --allyesconfig          New config where all options are answered "
      "with yes\n");
  printf(
      "  --allmodconfig          New config where all options are answered "
      "with mod\n");
  printf(
      "  --alldefconfig          New config with all symbols set to default\n");
  printf(
      "  --randconfig            New config with random answer to all "
      "options\n");
  printf(
  ...
  switch (input_mode) {
    ...
    case randconfig:
      ...
      if (conf_read_simple(name, S_DEF_USER) &&
        ...
        fprintf(stderr,
                "*** KCONFIG_ALLCONFIG set, but no \"%s\" or \"all.config\" "
                "file found\n",
                name);
        exit(1);
      }
