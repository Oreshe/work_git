...
static struct module* find_module(const char* filename, const char* modname) {
  ...
  list_for_each_entry(mod, &modules, list) {
    if (!strcmp(mod->dump_file, filename) && !strcmp(mod->name, modname))
      return mod;
  }
  return NULL;
}
...
static void sym_add_unresolved(const char* name,
                               struct module* mod,
                               bool weak) {
  struct symbol* sym;
...
static int ignore_undef_symbol(struct elf_info* info, const char* symname) {
  ...
  if (info->hdr->e_machine == EM_PPC)
     ...
     */
    if (strstarts(symname, "_restgpr_") || strstarts(symname, "_savegpr_") ||
        strstarts(symname, "_rest32gpr_") ||
        strstarts(symname, "_save32gpr_") || strstarts(symname, "_restvr_") ||
        strstarts(symname, "_savevr_"))
      return 1;
  if (info->hdr->e_machine == EM_PPC64)
    /* Special register function linked on all modules during final link of .ko
     */
    if (strstarts(symname, "_restgpr0_") || strstarts(symname, "_savegpr0_") ||
        strstarts(symname, "_restgpr1_") || strstarts(symname, "_savegpr1_") ||
        strstarts(symname, "_restfpr_") || strstarts(symname, "_savefpr_") ||
        strstarts(symname, "_restvr_") || strstarts(symname, "_savevr_") ||
...
static void handle_symbol(struct module* mod,
...
static char* get_next_modinfo(struct elf_info* info,
                              const char* tag,
                              char* prev) {
  char* p;
 ...
 * without "ax" / "aw".
 */
static void check_section(const char* modname,
                          struct elf_info* elf,
                          Elf_Shdr* sechdr) {
  const char* sec = sech_name(elf, sechdr);

  if (sechdr->sh_type == SHT_PROGBITS && !(sechdr->sh_flags & SHF_ALLOC) &&
...
#define ALL_PCI_INIT_SECTIONS                                              \
  ".pci_fixup_early", ".pci_fixup_header", ".pci_fixup_final",             \
      ".pci_fixup_enable", ".pci_fixup_resume", ".pci_fixup_resume_early", \
      ".pci_fixup_suspend"
...
#define OTHER_TEXT_SECTIONS                                             \
  ".ref.text", ".head.text", ".spinlock.text", ".fixup", ".entry.text", \
      ".exception.text", ".coldtext", ".softirqentry.text", ".irqentry.text"
 ...
 *
 **/
static int secref_whitelist(const char* fromsec,
                            const char* fromsym,
                            const char* tosec,
                            const char* tosym) {
  /* Check for pattern 1 */
  if (match(tosec, PATTERNS(ALL_INIT_DATA_SECTIONS)) &&
      match(fromsec, PATTERNS(DATA_SECTIONS)) && strstarts(fromsym, "__param"))
    return 0;
...
static Elf_Sym* find_fromsym(struct elf_info* elf,
                             Elf_Addr addr,
                             unsigned int secndx) {
...
static Elf_Sym* find_tosym(struct elf_info* elf, Elf_Addr addr, Elf_Sym* sym) {
   ...
   */
  new_sym = symsearch_find_nearest(elf, addr, get_secindex(elf, sym), true, 20);
...
static void default_mismatch_handler(const char* modname,
                                     struct elf_info* elf,
  ...
  /*
   * The format for the reference source:      <symbol_name>+<offset> or
   * <address> The format for the reference destination: <symbol_name> or
   * <address>
   */
  warn(
      "%s: section mismatch in reference: %s%s0x%x (section: %s) -> %s "
      "(section: %s)\n",
...
static void check_export_symbol(struct module* mod,
                                struct elf_info* elf,
                                Elf_Addr faddr,
                                const char* secname,
                                Elf_Sym* sym) {
  ...
  if (strcmp(label_name + strlen(prefix), name)) {
    error(
        "%s: .export_symbol section references '%s', but it does not seem to "
        "be an export symbol\n",
        mod->name, name);
    return;
  }
  ...
  if (match(secname, PATTERNS(ALL_INIT_SECTIONS)))
    warn(
        "%s: %s: EXPORT_SYMBOL used for init symbol. Remove __init or "
        "EXPORT_SYMBOL.\n",
        mod->name, name);
  else if (match(secname, PATTERNS(ALL_EXIT_SECTIONS)))
    warn(
        "%s: %s: EXPORT_SYMBOL used for exit symbol. Remove __exit or "
        "EXPORT_SYMBOL.\n",
        mod->name, name);
}

static void check_section_mismatch(struct module* mod,
                                   struct elf_info* elf,
                                   Elf_Sym* sym,
                                   unsigned int fsecndx,
                                   const char* fromsec,
  ...
  default_mismatch_handler(mod->name, elf, mismatch, sym, fsecndx, fromsec,
                           faddr, tosec, taddr);
}
...
static Elf_Addr addend_arm_rel(void* loc, Elf_Sym* sym, unsigned int r_type) {
  ...
  switch (r_type) {
    ...
    case R_ARM_MOVT_ABS:
      inst = get_unaligned_native((uint32_t*)loc);
      offset = sign_extend32(((inst & 0xf0000) >> 4) | (inst & 0xfff), 15);
    ...
    case R_ARM_THM_MOVT_ABS:
      ...
      offset =
          sign_extend32(((upper & 0x000f) << 12) | ((upper & 0x0400) << 1) |
    ...
    case R_ARM_THM_JUMP19:
      ...
      j2 = (lower >> 11) & 1;
      offset =
          sign_extend32((sign << 20) | (j2 << 19) | (j1 << 18) |
                            ((upper & 0x03f) << 12) | ((lower & 0x07ff) << 1),
                        20);
    ...
    case R_ARM_THM_JUMP24:
      ...
      j2 = (lower >> 11) & 1;
      offset =
          sign_extend32((sign << 24) | ((~(j1 ^ sign) & 1) << 23) |
                            ((~(j2 ^ sign) & 1) << 22) |
...
static void get_rel_type_and_sym(struct elf_info* elf,
                                 uint64_t r_info,
                                 unsigned int* r_type,
                                 unsigned int* r_sym) {
  typedef struct {
...
static void section_rela(struct module* mod,
                         struct elf_info* elf,
                         unsigned int fsecndx,
                         const char* fromsec,
  ...
  for (rela = start; rela < stop; rela++) {
    ...
    switch (elf->hdr->e_machine) {
      case EM_RISCV:
        if (!strcmp("__ex_table", fromsec) && r_type == R_RISCV_SUB32)
          continue;
        break;
      case EM_LOONGARCH:
        switch (r_type) {
    ...
    check_section_mismatch(mod, elf, tsym, fsecndx, fromsec, r_offset, taddr);
  }
}

static void section_rel(struct module* mod,
                        struct elf_info* elf,
                        unsigned int fsecndx,
                        const char* fromsec,
                        const Elf_Rel* start,
  ...
  for (i = 0; i < elf->num_sections; i++) {
    ...
    if (sechdr->sh_type == SHT_REL || sechdr->sh_type == SHT_RELA) {
      ...
      if (sechdr->sh_type == SHT_RELA)
        section_rela(mod, elf, secndx, secname, start, stop);
      else
        section_rel(mod, elf, secndx, secname, start, stop);
    }
  }
}
...
static void extract_crcs_for_object(const char* object, struct module* mod) {
  ...
  ret =
      snprintf(cmd_file, sizeof(cmd_file), "%.*s.%s.cmd", dirlen, object, base);
...
static void read_symbols(const char* modname) {
  ...
  if (info.no_trim_symbol_len) {
    ...
    memcpy(mod->no_trim_symbol, info.no_trim_symbol, info.no_trim_symbol_len);
 ...
 */
static bool verify_module_namespace(const char* namespace,
                                    const char* modname) {
  ...
  list_for_each_entry(s, &mod->unresolved_symbols, list) {
    ...
    if (!verify_module_namespace(exp->namespace, basename) &&
      ...
      modpost_log(!allow_missing_ns_imports,
                  "module %s uses symbol %s from namespace %s, but does not "
                  "import it.\n",
...
static void add_exported_symbols(struct buffer* buf, struct module* mod) {
  ...
  list_for_each_entry(sym, &mod->exported_symbols, list) {
    ...
    if (!sym->crc_valid)
      warn(
          "EXPORT symbol \"%s\" [%s%s] version generation failed, symbol will "
          "not be versioned.\n"
          "Is \"%s\" prototyped in <asm/asm-prototypes.h>?\n",
          sym->name, mod->name, mod->is_vmlinux ? "" : ".ko", sym->name);
  ...
  list_for_each_entry(mod, &modules, list) {
    ...
    list_for_each_entry(sym, &mod->exported_symbols, list) {
      ...
      buf_printf(&buf, "0x%08x\t%s\t%s\tEXPORT_SYMBOL%s\t%s\n", sym->crc,
                 sym->name, mod->name, sym->is_gpl_only ? "_GPL" : "",
                 sym->namespace);
    }
  }
  write_buf(&buf, fname);
...
int main(int argc, char** argv) {
  ...
  if (sec_mismatch_count && !sec_mismatch_warn_only)
    error(
        "Section mismatches detected.\n"
        "Set CONFIG_SECTION_MISMATCH_WARN_ONLY=y to allow them.\n");

  if (nr_unresolved > MAX_UNRESOLVED_REPORTS)
    warn(
        "suppressed %u unresolved symbol warnings because there were too "
        "many)\n",
