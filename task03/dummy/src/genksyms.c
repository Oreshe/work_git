...
static const struct {
  ...
  const char* name;
    ...
    [SYM_NORMAL] = {0, NULL},     [SYM_TYPEDEF] = {'t', "typedef"},
    [SYM_ENUM] = {'e', "enum"},   [SYM_STRUCT] = {'s', "struct"},
    [SYM_UNION] = {'u', "union"}, [SYM_ENUM_CONST] = {'E', "enum constant"},
};
...
static int is_unknown_symbol(struct symbol* sym) {
  ...
  return ((sym->type == SYM_STRUCT || sym->type == SYM_UNION ||
          ...
          defn->tag == SYM_NORMAL && strcmp(defn->string, "UNKNOWN") == 0 &&
          (defn = defn->next) && defn->tag == SYM_NORMAL &&
          strcmp(defn->string, "{") == 0);
}

static struct string_list* process_enum(const char* name,
  ...
  if (type == SYM_ENUM_CONST) {
    if (defn) {
      ...
      if (last_enum_expr) {
        ...
        defn = concat_list(mk_node("("), expr, mk_node(")"), mk_node("+"),
                           mk_node(buf), NULL);
      } else {
        defn = mk_node(buf);
      }
...
static struct symbol* __add_symbol(const char* name,
                                   enum symbol_type type,
                                   struct string_list* defn,
                                   int is_extern,
                                   int is_reference) {
  ...
  hash_for_each_possible(symbol_hashtable, sym, hnode, h) {
    if (map_to_ns(sym->type) != map_to_ns(type) || strcmp(name, sym->name))
      continue;
    ...
    if (is_reference) {
      ...
      status = is_unknown_symbol(sym) ? STATUS_DEFINED : STATUS_MODIFIED;
      break;
    }
    free_list(defn, NULL);
    return sym;
  ...
  if (flag_debug) {
    if (symbol_types[type].name)
      fprintf(debugfile, "Defn for %s %s == <", symbol_types[type].name, name);
    else
      fprintf(debugfile, "Defn for type%d %s == <", type, name);
    if (is_extern)
...
static void read_reference(FILE* f) {
  while (!feof(f)) {
    ...
    sym = read_node(f);
    if (sym && sym->tag == SYM_NORMAL && !strcmp(sym->string, "override")) {
      is_override = 1;
      free_node(sym);
      sym = read_node(f);
    }
    if (!sym)
      continue;
    def = read_node(f);
    if (def && def->tag == SYM_NORMAL && !strcmp(def->string, "extern")) {
      is_extern = 1;
      free_node(def);
      def = read_node(f);
    }
    ...
    subsym = add_reference_symbol(sym->string, sym->tag, defn, is_extern);
    ...
    switch (cur->tag) {
      ...
      case SYM_ENUM:
        ...
        if (!subsym) {
          ...
          error_with_pos("expand undefined %s %s", symbol_types[cur->tag].name,
                         cur->string);
          n = concat_list(mk_node(symbol_types[cur->tag].name),
        ...
        if (subsym->expansion_trail) {
          if (flag_dump_defs) {
            fprintf(debugfile, "%s %s ", symbol_types[cur->tag].name,
                    cur->string);
          }

          crc = partial_crc32(symbol_types[cur->tag].name, crc);
          crc = partial_crc32_one(' ', crc);
          crc = partial_crc32(cur->string, crc);
          crc = partial_crc32_one(' ', crc);
        } else {
...
void export_symbol(const char* name) {
  ...
  while (sym != (struct symbol*)-1L) {
    ...
    if (sym->status != STATUS_UNCHANGED) {
      if (!has_changed) {
        print_location();
        fprintf(stderr, "%s: %s: modversion changed because of changes in ",
                flag_preserve ? "error" : "warning", name);
      } else {
        fprintf(stderr, ", ");
      }
...
int main(int argc, char** argv) {
  ...
  while ((o = getopt_long(argc, argv, "dwqVDr:T:ph", &long_opts[0], NULL)) !=
         EOF)
    switch (o) {
      case 'd':
        flag_debug++;
