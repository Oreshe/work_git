...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i <= MAX_PACKED_FIELD_SIZE; i++)
    printf(
        "\t__builtin_choose_expr(ARRAY_SIZE(fields) == %d, ({ "
        "CHECK_PACKED_FIELDS_%d(fields); }), \\\n",
        i, i);

  printf(
      "\t({ BUILD_BUG_ON_MSG(1, \"CHECK_PACKED_FIELDS() must be regenerated to "
      "support array sizes larger than %d.\"); }) \\\n",
